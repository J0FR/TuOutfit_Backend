...
public class OutfitService {
    ...
    @Transactional
    public OutfitEntity updateOutfit(Long outfitId, OutfitEntity outfit) throws EntityNotFoundException, IllegalOperationException {
            log.info("Inicia proceso de actualizar el libro con id = {0}", outfitId);
            Optional<OutfitEntity> outfitEntity = outfitRepository.findById(outfitId);
            if (outfitEntity.isEmpty())
                    throw new EntityNotFoundException(ErrorMessage.OUTFIT_NOT_FOUND);

            outfit.setId(outfitId);
            log.info("Termina proceso de actualizar el libro con id = {0}", outfitId);
            return outfitRepository.save(outfit);
    }

    @Transactional
    public void deleteOutfit(Long outfitId) throws EntityNotFoundException, IllegalOperationException {
            log.info("Inicia proceso de borrar el libro con id = {0}", outfitId);
            Optional<OutfitEntity> bookEntity = outfitRepository.findById(outfitId);
            if (bookEntity.isEmpty())
                    throw new EntityNotFoundException(ErrorMessage.OUTFIT_NOT_FOUND);

            outfitRepository.deleteById(outfitId);
            log.info("Termina proceso de borrar el libro con id = {0}", outfitId);
    }
